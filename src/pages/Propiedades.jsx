...
import { Search, Filter, RefreshCw, Frown, Building, MapPin, DollarSign, Bed, SlidersHorizontal, ChevronDown } from 'lucide-react';
...
export default function Propiedades() {
  ...
  const filteredProperties = useMemo(() => {
    return propertiesData.filter((property) => {
      ...
      // Operation filter
      if (operation !== 'Todos' && property.operation !== operation) {
        return false;
      }

      // Type filter
      if (type !== 'Todos' && property.type !== type) {
        return false;
      }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-6">
        <div>
          <span className="text-xs font-bold uppercase tracking-wider text-sky-600">Bienes Raíces</span>
        ...
        <div className="flex items-center gap-3">
          <div className="bg-sky-50 text-sky-700 font-semibold px-4 py-2.5 rounded-2xl border border-sky-100 text-sm shadow-sm">
            Mostrando <span className="font-extrabold text-sky-900">{filteredProperties.length}</span> {filteredProperties.length === 1 ? 'propiedad encontrada' : 'propiedades encontradas'}
          </div>

          <button
            onClick={() => setIsMobileFilterOpen(!isMobileFilterOpen)}
            className="lg:hidden inline-flex items-center gap-2 bg-slate-900 text-white px-4 py-2.5 rounded-2xl text-sm font-medium shadow-md"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        <aside 
          ...
          } bg-white p-6 rounded-3xl border border-slate-100 shadow-xl lg:shadow-sm space-y-6 sticky top-24 z-30`}
        >
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:bg-white transition-all"
              />
            </div>
          </div>

          {/* 2. Operación (Venta / Alquiler / Todos) */}
          <div className="space-y-2">
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-500">
              Operación
            </label>
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:bg-white transition-all"
            >
          ...
          {hasActiveFilters && (
            <button
              onClick={handleResetFilters}
              className="w-full inline-flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold py-2.5 rounded-xl text-xs transition-colors pt-3 border border-slate-200"
            >
              <RefreshCw className="w-3.5 h-3.5" />
