...
import { 
  ...
  Car, Compass, CheckCircle2, Building2, Send, ShieldCheck, Share2, Heart 
} from 'lucide-react';
...
export default function DetallePropiedad() {
  ...
  // Active image state for main gallery viewer
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-6 md:p-10 shadow-sm space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 border-b border-slate-100 pb-6">
          ...
          <div className="bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100 shrink-0 text-left md:text-right">
        ...
        {/* 2. Galería de Fotos (Airbnb-style Grid & Interactive Viewer) */}
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-[380px] sm:h-[480px] rounded-3xl overflow-hidden shadow-inner">
            {/* Main Featured Image */}
            <div className="md:col-span-2 h-full bg-slate-100 relative group overflow-hidden">
              <img 
                src={selectedImage} 
                alt={title}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 pt-4">
          ...
          {/* Main Specs & Description (Left Column - 2 Cols wide) */}
          <div className="lg:col-span-2 space-y-8">
            
            {/* Ficha Técnica (Grid de Características) */}
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-slate-900 border-b border-slate-100 pb-3">
                Ficha Técnica del Inmueble
              </h2>
              
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[11px] text-slate-400 font-semibold block">Dormitorios</span>
                    <span className="font-bold text-slate-800 text-sm">{bedrooms > 0 ? `${bedrooms} Ambs.` : 'Monoamb.'}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[11px] text-slate-400 font-semibold block">Baños</span>
                    <span className="font-bold text-slate-800 text-sm">{bathrooms}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[11px] text-slate-400 font-semibold block">Cocheras</span>
                    <span className="font-bold text-slate-800 text-sm">{parking > 0 ? `${parking} Fija` : 'Sin cochera'}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[11px] text-slate-400 font-semibold block">Sup. Total</span>
                    <span className="font-bold text-slate-800 text-sm">{area} m²</span>
                  </div>
                </div>
                ...
                {coveredArea && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-[11px] text-slate-400 font-semibold block">Sup. Cubierta</span>
                      <span className="font-bold text-slate-800 text-sm">{coveredArea} m²</span>
                    </div>
                  </div>
                )}
                ...
                {disposition && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-[11px] text-slate-400 font-semibold block">Disposición</span>
                      <span className="font-bold text-slate-800 text-sm">{disposition}</span>
                    </div>
                  </div>
                )}
                ...
                {age && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-[11px] text-slate-400 font-semibold block">Antigüedad</span>
                      <span className="font-bold text-slate-800 text-sm">{age}</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            {amenities && amenities.length > 0 && (
              <div className="space-y-4 pt-2">
                <h3 className="text-lg font-bold text-slate-900">Comodidades y Servicios</h3>
          ...
          {/* Tarjeta Lateral de Contacto / Agente (Right Column - Sticky) */}
          <div className="space-y-6 sticky top-24">
            <div className="bg-slate-900 text-white p-6 rounded-3xl space-y-6 shadow-xl border border-slate-800">
              ...
              {/* Direct WhatsApp Button with Property Title */}
              <div className="pt-2">
                <a
